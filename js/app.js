...
// ==================== AI 潤稿功能 ====================
  const POLISH_PROMPTS = {
    fluency: '請在不改變原意的情況下，優化以下中文文章的語句流暢度和用詞。保持原有結構，只潤飾文句。完成後字數至少要有 400 字（移除空白後計算）。直接回傳潤飾後的全文，不要加入任何說明：',
    concise: '請精簡以下中文文章，移除重複或冗餘的句子，但保留所有重要觀點。完成後字數至少要有 400 字（移除空白後計算）。直接回傳精簡後的全文，不要加入任何說明：',
    expand: '請在不改變原意的情況下，為以下中文文章補充更多細節與描寫，使內容更豐富。完成後字數至少要有 400 字（移除空白後計算）。直接回傳擴寫後的全文，不要加入任何說明：',
    formal: '請將以下中文文章轉換為較正式的書面語氣，適合投稿或學術用途。完成後字數至少要有 400 字（移除空白後計算）。直接回傳轉換後的全文，不要加入任何說明：'
  };
